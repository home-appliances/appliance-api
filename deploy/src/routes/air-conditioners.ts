...
app.get('/', async (c) => {
  ...
  const countResult = await pool.query(
    `SELECT COUNT(*) FROM ac_images ${where}`,
    params
  );
...
app.get('/:id/image', async (c) => {
  ...
  const result = await pool.query(
    'SELECT main_image_data, main_image_type FROM ac_images WHERE id = $1',
    [id]
  );
...
app.get('/uid/:uid/image', async (c) => {
  ...
  const result = await pool.query(
    'SELECT main_image_data, main_image_type FROM ac_images WHERE uid = $1',
    [uid]
  );
...
app.get('/stats/summary', async (c) => {
  const [total, withImage, brands] = await Promise.all([
    pool.query('SELECT COUNT(*) FROM ac_images'),
    pool.query('SELECT COUNT(*) FROM ac_images WHERE main_image_data IS NOT NULL'),
    pool.query('SELECT COUNT(DISTINCT brand_en) FROM ac_images'),
  ]);
