...
// 所有爬虫路由都需要认证
crawler.use('/api/admin/crawler/*', authMiddleware);
...
crawler.get('/api/admin/crawler/tasks', async (c) => {
  try {
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) FROM crawler_tasks ${whereClause}`,
      params
    );
...
crawler.post('/api/admin/crawler/start', async (c) => {
  try {
    ...
    const runningTask = await pool.query(
      "SELECT id FROM crawler_tasks WHERE status = 'running' LIMIT 1"
    );
    ...
    // 创建新任务
    const result = await pool.query(
      `INSERT INTO crawler_tasks (category, status, started_at)
