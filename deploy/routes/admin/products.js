...
products.get('/api/admin/products', async (c) => {
    try {
        ...
        // 查询总数
        const countResult = await index_js_1.pool.query(`SELECT COUNT(*) FROM products p ${whereClause}`, params);
        const total = parseInt(countResult.rows[0].count);
        // 查询数据（关联 categories 表获取分类信息）
        const dataResult = await index_js_1.pool.query(`SELECT p.id, p.name, p.brand, p.category_id, c.name as category_name, c.code as category_code,
...
products.post('/api/admin/products', async (c) => {
    try {
        ...
        const result = await index_js_1.pool.query(`INSERT INTO products (name, brand, category_id, model, price, original_price, rating, review_count, params, source_platform)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'admin')
       RETURNING id, name, brand, category_id, model, price, original_price, rating, review_count, created_at`, [name, brand, category_id || null, model || null, price || null, original_price || null, rating || null, review_count || 0, params || '{}']);
...
products.get('/api/admin/products/:id', async (c) => {
    try {
        ...
        const result = await index_js_1.pool.query(`SELECT p.*, c.name as category_name, c.code as category_code
       FROM products p
       LEFT JOIN categories c ON c.id = p.category_id
       WHERE p.id = $1`, [id]);
        ...
        // 获取产品图片
        const images = await index_js_1.pool.query(`SELECT * FROM product_images WHERE product_id = $1 ORDER BY image_type, sort_order`, [id]);
        return c.json({
            code: 0,
            data: {
                ...result.rows[0],
...
products.put('/api/admin/products/:id', async (c) => {
    try {
       ...
       WHERE id = $10
       RETURNING id, name, brand, category_id, model, price, original_price, rating, review_count, params, updated_at`, [name, brand, category_id, model, price, original_price, rating, review_count, productParams ? JSON.stringify(productParams) : null, id]);
        if (result.rows.length === 0) {
            return c.json({ code: 404, message: '产品不存在' }, 404);
        }
