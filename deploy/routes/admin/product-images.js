...
productImages.get('/api/admin/product-images/:id', async (c) => {
    try {
        ...
        const result = await index_js_1.pool.query('SELECT * FROM product_images WHERE id = $1', [id]);
...
productImages.post('/api/admin/product-images', async (c) => {
    try {
        ...
        const result = await index_js_1.pool.query(`INSERT INTO product_images (product_id, image_url, image_type, sort_order)
       VALUES ($1, $2, $3, $4)
       RETURNING *`, [product_id, image_url || null, image_type || 'main', sort_order || 0]);
...
productImages.put('/api/admin/product-images/:id', async (c) => {
    try {
        ...
        const result = await index_js_1.pool.query(`UPDATE product_images
       SET image_url = COALESCE($1, image_url),
           image_type = COALESCE($2, image_type),
           sort_order = COALESCE($3, sort_order)
       WHERE id = $4
       RETURNING *`, [image_url, image_type, sort_order, id]);
        if (result.rows.length === 0) {
            return c.json({ code: 404, message: '图片不存在' }, 404);
        }
...
productImages.delete('/api/admin/product-images/:id', async (c) => {
    try {
        const id = c.req.param('id');
        const result = await index_js_1.pool.query('DELETE FROM product_images WHERE id = $1 RETURNING id', [id]);
        if (result.rows.length === 0) {
            return c.json({ code: 404, message: '图片不存在' }, 404);
        }
...
productImages.put('/api/admin/product-images/batch/sort', async (c) => {
    try {
        ...
        for (const item of items) {
            await index_js_1.pool.query('UPDATE product_images SET sort_order = $1 WHERE id = $2', [item.sort_order, item.id]);
        }
...
productImages.post('/api/admin/product-images/batch/delete', async (c) => {
    try {
        ...
        const result = await index_js_1.pool.query('DELETE FROM product_images WHERE id = ANY($1) RETURNING id', [ids]);
        return c.json({
            code: 0,
