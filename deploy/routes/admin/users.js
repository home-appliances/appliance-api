...
// 所有用户路由需要认证
users.use('/api/admin/users/*', auth_js_1.authMiddleware);
/**
 * 获取用户列表
...
users.get('/api/admin/users', async (c) => {
    try {
        ...
        const countResult = await index_js_1.pool.query(countQuery, countParams);
...
users.get('/api/admin/users/:id', async (c) => {
    try {
        ...
        const result = await index_js_1.pool.query('SELECT id, username, name, email, phone, role, status, avatar, remark, created_at, last_login FROM admins WHERE id = $1', [id]);
...
users.post('/api/admin/users', async (c) => {
    try {
        ...
        // 检查用户名是否已存在（排除已删除的）
        const exists = await index_js_1.pool.query("SELECT id FROM admins WHERE username = $1 AND status != 'deleted'", [username]);
        if (exists.rows.length > 0) {
            return c.json({ code: 400, message: '用户名已存在' }, 400);
        }
        const passwordHash = await bcryptjs_1.default.hash(password, 10);
        const avatar = name.slice(0, 2);
        const result = await index_js_1.pool.query(`INSERT INTO admins (username, name, email, phone, password_hash, role, status, avatar, remark)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING id, username, name, email, phone, role, status, avatar, remark, created_at`, [username, name, email || null, phone || null, passwordHash, role || 'admin', status || 'active', avatar, remark || null]);
...
users.delete('/api/admin/users/:id', async (c) => {
    try {
        ...
        const result = await index_js_1.pool.query("UPDATE admins SET status = 'deleted', username = username || $1, updated_at = NOW() WHERE id = $2 RETURNING id", [deletedSuffix, id]);
        if (result.rows.length === 0) {
            return c.json({ code: 404, message: '用户不存在' }, 404);
        }
...
users.put('/api/admin/users/:id/status', async (c) => {
    try {
        ...
        const result = await index_js_1.pool.query('UPDATE admins SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING id, status', [status, id]);
        if (result.rows.length === 0) {
            return c.json({ code: 404, message: '用户不存在' }, 404);
        }
...
users.put('/api/admin/users/:id/reset-password', async (c) => {
    try {
        const id = parseInt(c.req.param('id'));
        const newHash = await bcryptjs_1.default.hash('123456', 10);
        await index_js_1.pool.query('UPDATE admins SET password_hash = $1, updated_at = NOW() WHERE id = $2', [newHash, id]);
