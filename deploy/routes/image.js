...
image.post('/api/image/download', async (c) => {
    try {
        ...
        const existing = await index_js_1.pool.query('SELECT id FROM images WHERE source_url = $1', [url]);
    ...
    `, [buffer, mimeType, buffer.length, url]);
        ...
        if (product_id) {
            await index_js_1.pool.query('UPDATE products SET image_id = $1 WHERE id = $2', [imageId, product_id]);
...
image.post('/api/image/upload', async (c) => {
    ...
    `, [buffer, mimeType, buffer.length]);
        ...
        if (productId) {
            await index_js_1.pool.query('UPDATE products SET image_id = $1 WHERE id = $2', [imageId, parseInt(productId)]);
        }
        return c.json({
            code: 0,
            message: '图片上传成功',
...
image.get('/api/image/:id', async (c) => {
    try {
        ...
        // 从数据库读取图片
        const result = await index_js_1.pool.query('SELECT image_data, mime_type FROM images WHERE id = $1', [id]);
...
image.delete('/api/image/:id', async (c) => {
    try {
        const id = parseInt(c.req.param('id'));
        // 先取消产品关联
        await index_js_1.pool.query('UPDATE products SET image_id = NULL WHERE image_id = $1', [id]);
        // 删除图片
        const result = await index_js_1.pool.query('DELETE FROM images WHERE id = $1 RETURNING id', [id]);
        if (result.rows.length === 0) {
            return c.json({ error: '图片不存在' }, 404);
        }
        return c.json({
